...
def convert_examples_to_features(examples, label_list, max_seq_length, tokenizer):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        padding_roberta = [1] * (max_seq_length - len(input_ids))
        input_ids += padding_roberta
        input_mask += padding
        segment_ids += padding
...
def main():
    ...
    parser.add_argument("--bert_model", default=None, type=str, required=True,
                        help="Bert pre-trained model selected in the list: roberta-base, xlm-roberta-base")
    ...
    if args.bert_model == "roberta-base":
        ...
        model = XLMRobertaForSequenceClassification.from_pretrained(args.bert_model,
                num_labels = num_labels)   
    else:
    ...
    if args.do_train:
        ...
        for _ in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm_bar):
                ...
                class_outputs = model(input_ids, token_type_ids=segment_ids, attention_mask=input_mask, labels=label_ids)
                loss = class_outputs.loss
                if n_gpu > 1:
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        for input_ids, input_mask, segment_ids, label_ids in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                class_eval_outputs = model(input_ids, token_type_ids=segment_ids, attention_mask=input_mask, labels=label_ids, output_attentions=True)
                tmp_eval_loss = class_eval_outputs.loss
                logits = class_eval_outputs.logits
                attentions = torch.mean(class_eval_outputs.attentions[-1], dim=1)
